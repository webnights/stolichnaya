...
app.get('/buses', (req, res) => {
  ...
  connection.query(sql, (err, data) => {
    ...
    data.forEach(row => {
      const { bus_id, bus_name, small_description, bus_image, bus_description, bus_type, bus_price, param, param_value } = row;
...
app.get('/cart/:userId', (req, res) => {
  ...
  const sql = `   SELECT 
    ...
    buses.id as bus_id,
    buses.name as product_name,
    buses.image as product_image,
    buses.price as product_price
    FROM carts
    INNER JOIN buses ON carts.product_id=buses.id
    WHERE carts.user_id = ?`
  try {
